...
storage[HUD_PANEL_STORAGE] = storage[HUD_PANEL_STORAGE] or { switches = { barLifeMana  = false, timerImbuiment = false, pushMax = false, targetInfo = false, deadTimers = false }}
...
local function deadTimersOn()
  return hudMasterOn() and deadTimersOn and deadTimersOn() == true
end

local DT_STORAGE = "deadTimersHUD"
storage[DT_STORAGE] = storage[DT_STORAGE] or {
  pos = { x = 300, y = 120 },
  list = {}
}
local dtConfig = storage[DT_STORAGE]

local DEAD_DURATION = 15 * 60

-- cria painel
local deadUI = setupUI([[
UIWindow
  id: deadTimersHUD
  width: 120
  height: 140
...
deadUI:setPosition({ x = dtConfig.pos.x, y = dtConfig.pos.y })

deadUI.onDragLeave = function(widget, droppedWidget, mousePos)
  local p = widget:getPosition()
  dtConfig.pos.x = p.x
  dtConfig.pos.y = p.y
end

deadUI.onMouseRelease = function(widget, mousePos, button)
  local p = widget:getPosition()
  dtConfig.pos.x = p.x
  dtConfig.pos.y = p.y
  return false
...
deadUI.onTouchRelease = function(widget, pos)
  local p = widget:getPosition()
  dtConfig.pos.x = p.x
  dtConfig.pos.y = p.y
  return false
...
local function rebuildList()
  deadUI.list:destroyChildren()

  local now = os.time()
  local any = false

  for name, exp in pairs(dtConfig.list) do
    local remain = exp - now
    if remain <= 0 then
      dtConfig.list[name] = nil
    else
      any = true
      local lbl = g_ui.createWidget("Label", deadUI.list)
      lbl:setFont("verdana-9px")
      lbl:setColor("white")
      lbl:setText(name .. ": " .. fmtTime(remain))
    end
  end
  ...
  if not any then
    ...
    lbl:setFont("verdana-9px")
    lbl:setColor("gray")
    lbl:setText("Sem mortes recentes")
  end
end
...
macro(250, function()
  if not hudMasterOn() or not (storage[HUD_PANEL_STORAGE] and storage[HUD_PANEL_STORAGE].switches and storage[HUD_PANEL_STORAGE].switches.deadTimers) then
    deadUI:hide()
    return
  end

  if not deadUI:isVisible() then deadUI:show() end
  ...
  for cid, data in pairs(prevPlayers) do
    if not curr[cid] then
      if data and data.name and data.name ~= "" and data.hp and data.hp <= 1 then
        dtConfig.list[data.name] = os.time() + DEAD_DURATION
      end
    end
  end
